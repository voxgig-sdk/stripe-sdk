...
class StripeMakeError
{
    public static function call(?StripeContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new StripeContext([], null);
        }
        $op = $ctx->op ?? new StripeOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new StripeResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof StripeError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new StripeError('', $msg, $ctx);
