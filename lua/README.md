...
It exposes the API as capitalised, semantic **Entities** — e.g. `client:Session()` — each with the same small set of operations (`list`, `load`, `create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
```lua
-- Create
local created, err = client:Session():create({ amount_total = 1, cancel_url = "example_cancel_url" })
if err then error(err) end
