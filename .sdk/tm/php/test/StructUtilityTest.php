...
// Stripe SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = StripeOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(StripeOmni::structfix($now, Struct::undef()))
                        !== json_encode(StripeOmni::structfix($val, Struct::undef()))) {
