...
const TEST_SEED = { "entity": { "session": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+StripeSDK\b/.test(code) ||
        /\bStripeSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+StripeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/StripeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
