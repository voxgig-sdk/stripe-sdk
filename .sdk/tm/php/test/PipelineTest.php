...
{
    private static function utility(): StripeUtility
    {
        return new StripeUtility();
    }
    ...
    private static function ctx(array $over = []): StripeContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new StripeContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new StripeOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof StripeError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new StripeOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = StripeMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new StripeOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = StripeMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new StripeOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = StripeMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StripeSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = StripeMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        ...
        [, $err] = StripeMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new StripeSpec(['step' => 's']),
            'response' => new StripeResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new StripeResult([]),
        ]);
        [, $err] = StripeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new StripeSpec(['step' => 's']),
            'response' => new StripeResponse(self::resp(200, ['v' => 1])),
            'result' => new StripeResult([]),
        ]);
        [, $err] = StripeMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new StripeSpec(['step' => 's']),
            'response' => new StripeResponse(self::resp(200, ['v' => 2])),
            'result' => new StripeResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        StripeMakeResponse::call($ctx);
    ...
    {
        $preset = new StripeResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new StripeSpec([]),
            'response' => new StripeResponse([]),
            'result' => new StripeResult([]),
        ...
        [$got, $err] = StripeMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new StripeResult([])]);
        [, $err] = StripeMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new StripeSpec([]), 'result' => null]);
        [, $err] = StripeMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StripeSpec(['step' => 's']),
            'result' => new StripeResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new StripeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StripeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new StripeSpec(['step' => 's']),
            'result' => new StripeResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new StripeOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = StripeMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new StripeResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new StripeSpec([]),
            'result' => new StripeResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = StripeMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StripeMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new StripeError('boom', 'boom');
        $utility->fetcher = function (StripeContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StripeSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = StripeMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (StripeContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = StripeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(StripeResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (StripeContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new StripeSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = StripeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new StripeResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new StripeSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = StripeMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = StripeMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = StripeMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new StripeResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, StripeDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new StripeResult(['ok' => false])]);
        $this->expectException(StripeError::class);
        StripeDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new StripeResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', StripeMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new StripeBaseFeature();
        $b = new StripeBaseFeature();
        StripeFeatureAdd::call($ctx, $a);
        StripeFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): StripeBaseFeature
    {
        $f = new StripeBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        StripeFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): StripeContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new StripeSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = StripePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new StripeResponse(['headers' => null]),
            'result' => new StripeResult([]),
        ]);
        StripeResultHeaders::call($ctx);
