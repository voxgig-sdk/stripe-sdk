...
class SessionEntity
{
    ...
    {
        if ($args) {
            $this->_data = StripeHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = StripeHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *
     * @param SessionLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed SessionLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Session|array The loaded Session as an assoc-array at the
     *   SDK boundary; throws StripeError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = StripeHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws StripeError on failure (item-5 convention).
     */
     ...
     *
     * @param SessionCreateData|array|null $reqdata Body data as an assoc-array;
     *   a typed SessionCreateData names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Session|array The created Session as an assoc-array at the
     *   SDK boundary; throws StripeError on failure (item-5 convention).
     */
