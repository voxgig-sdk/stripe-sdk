...
//    only - a typed StripeContext there would make every ctx
...
final class StripeOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new StripeContext($clean, null);
    ...
    public static function err_from_map($m): ?StripeError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new StripeError(is_string($code) ? $code : '', $msg);
    }
