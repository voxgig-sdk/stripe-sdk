...
import (
	...
	omni "github.com/voxgig-sdk/stripe-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.StripeError); is {
				out := map[string]any{
					"name":    "StripeError",
...
func omniCtx(arg any, client *sdk.StripeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.StripeSDK); is {
			client = live
