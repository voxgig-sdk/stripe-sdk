# Stripe TypeScript SDK



The TypeScript SDK for the Stripe API — a type-safe, entity-oriented client with full async/await support.

The API is exposed as capitalised, semantic **Entities** — e.g.
`client.Session()` — each with a small set of operations (`list`, `load`, `create`)
...
`list()` resolves to an array of Session ENTITIES — every operation
...
```ts
try {
  const session = await client.Session().load({ id: 'example_id' })
  console.log(session)
} catch (err) {
...
```ts
// Create — returns the created Session ENTITY (.data() for the record)
