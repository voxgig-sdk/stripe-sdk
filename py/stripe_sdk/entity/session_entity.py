...
from stripe_sdk.stripe_types import (
    Session,
    SessionLoadMatch,
    SessionListMatch,
    SessionCreateData,
...
class SessionEntity:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Session:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Session().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: SessionCreateData, ctrl=None) -> Session:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
