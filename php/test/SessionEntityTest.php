...
class SessionEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set STRIPE_TEST_SESSION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $session_ref01_data_result = $session_ref01_ent->create($session_ref01_data, null);
        $session_ref01_data = Helpers::to_map(is_object($session_ref01_data_result) && method_exists($session_ref01_data_result, 'data_get') ? $session_ref01_data_result->data_get() : $session_ref01_data_result);
        $this->assertNotNull($session_ref01_data);
        $this->assertNotNull($session_ref01_data["id"]);
        ...
        $session_ref01_data_dt0_loaded = $session_ref01_ent->load($session_ref01_match_dt0, null);
        $session_ref01_data_dt0_load_result = Helpers::to_map(is_object($session_ref01_data_dt0_loaded) && method_exists($session_ref01_data_dt0_loaded, 'data_get') ? $session_ref01_data_dt0_loaded->data_get() : $session_ref01_data_dt0_loaded);
        $this->assertNotNull($session_ref01_data_dt0_load_result);
        $this->assertEquals($session_ref01_data_dt0_load_result["id"], $session_ref01_data["id"]);
...
function session_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("STRIPE_TEST_SESSION_ENTID");
