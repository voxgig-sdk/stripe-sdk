...
The SDK exposes the API as capitalised, semantic **Entities** — for example `$client->Session()` — with named operations (`list`/`load`/`create`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
try {
    ...
    foreach ($sessions as $record) {
        ...
        echo $item["id"] . " " . $item["amount_total"] . "\n";
    }
} catch (\Throwable $err) {
...
```php
// create() returns the ENTITY — call data_get() for the created Session record.
$created = $client->Session()->create(["amount_total" => 1, "cancel_url" => "example_cancel_url"]);
...
print_r(array_map(fn($item) => $item->data_get(), $session));
```
