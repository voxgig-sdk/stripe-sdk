...
func TestSessionDirect(t *testing.T) {
	...
	t.Run("direct-load-session", func(t *testing.T) {
		setup := sessionDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-session", _mode); _shouldSkip {
			if _reason == "" {
				_reason = "skipped via sdk-test-control.json"
			}
			t.Skip(_reason)
...
func sessionDirectSetup(mockres any) *sessionDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"STRIPE_TEST_SESSION_ENTID": map[string]any{},
	...
	if live {
		...
		client := sdk.NewStripeSDK(mergedOpts)

		idmap := map[string]any{}
		if entidRaw, ok := env["STRIPE_TEST_SESSION_ENTID"]; ok {
