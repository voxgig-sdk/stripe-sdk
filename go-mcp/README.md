...
[MCP](https://modelcontextprotocol.io) server exposing the Stripe SDK as
two agent tools — `stripe_list` and `stripe_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/stripe-mcp)
make build
...
claude mcp add --scope user stripe \
  -- /absolute/path/to/stripe-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./stripe-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/stripe-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user stripe \
     -- "$PWD"/dist/*/stripe-mcp -transport stdio
   ```
...
| Tool | Args | Returns |
|------|------|---------|
| `stripe_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `stripe_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./stripe-mcp -transport http -addr :18080 &
