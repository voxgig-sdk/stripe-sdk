...
class FtCtrl extends StripeControl
{
...
class FtClient
{
    ...
    private StripeUtility $utility;

    public function __construct(StripeUtility $utility, array $options)
    {
    ...
    public function get_utility(): StripeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (StripeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (StripeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = StripeConfig::make_feature($name);
    ...
    private static function buildUrl(StripeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof StripeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new StripeResponse($fetched);
        }
    ...
    private function fail(StripeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (StripeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
