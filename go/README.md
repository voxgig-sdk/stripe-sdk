...
It exposes the API as capitalised, semantic **Entities** — e.g. `client.Session(nil)` — each with the same small set of operations (`List`, `Load`, `Create`) instead of raw URL paths and query strings. You call meaning, not endpoints, which keeps the cognitive load low.
...
func main() {
    ...
    // List session records — the value is the array of records itself.
    sessions, err := client.Session(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
    for _, item := range sessions.([]any) {
        fmt.Println(item)
    }

    // Load a single session — the value is the loaded record.
    session, err := client.Session(nil).Load(map[string]any{"id": "example_id"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(session)
    ...
    // Create a session.
    created, err := client.Session(nil).Create(map[string]any{"amount_total": 1, "cancel_url": "example_cancel_url"}, nil)
    if err != nil {
        panic(err)
    }
    fmt.Println(created)
...
slice):
    ...
    session, err := client.Session(nil).List(map[string]any{/* fields */}, nil)
    if err != nil { /* handle */ }
    // session is the returned record
